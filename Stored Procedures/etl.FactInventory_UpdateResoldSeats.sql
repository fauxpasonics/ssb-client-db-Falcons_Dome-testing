...
FROM (
	...
	WHERE activity = 'ES' AND tex.add_datetime > (GETDATE() - 2)
) a
WHERE RowRank = 1
...
, (tex.ResoldPurchasePrice + tex.ResoldFees) ResoldTotalAmount
	INTO #Tex_Update
FROM #Tex_Seats tex
INNER JOIN dbo.DimEvent de ON de.SSID_event_id = tex.event_id
...
LEFT OUTER JOIN dbo.DimCustomer dc ON dc.AccountId = tex.assoc_acct_id AND dc.SourceSystem = 'TM'
