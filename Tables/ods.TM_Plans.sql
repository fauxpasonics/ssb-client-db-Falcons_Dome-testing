...
[InsertDate] [datetime] NULL CONSTRAINT [DF__TM_Plans__Insert__6D0D32F4] DEFAULT (getdate()),
[UpdateDate] [datetime] NULL CONSTRAINT [DF__TM_Plans__Update__6E01572D] DEFAULT (getdate()),
...
GO
ALTER TABLE [ods].[TM_Plans] ADD CONSTRAINT [PK__TM_Plans__3213E83F36B84F64] PRIMARY KEY CLUSTERED  ([id])
GO
CREATE NONCLUSTERED INDEX [IDX_PlanKey] ON [ods].[TM_Plans] ([acct_id], [plan_event_id], [ticket_status], [section_id], [row_id], [seat_num]) INCLUDE ([num_seats], [ssbPriceCode])
GO
CREATE NONCLUSTERED INDEX [IDX_ticket_status] ON [ods].[TM_Plans] ([ticket_status]) INCLUDE ([event_id], [row_id], [seat_num], [section_name])
GO
CREATE NONCLUSTERED INDEX [IDX_UpdateDate] ON [ods].[TM_Plans] ([UpdateDate])
GO
