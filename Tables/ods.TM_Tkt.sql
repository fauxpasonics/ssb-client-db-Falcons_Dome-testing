...
[InsertDate] [datetime] NULL CONSTRAINT [DF__TM_Tkt__InsertDa__693CA210] DEFAULT (getdate()),
[UpdateDate] [datetime] NULL CONSTRAINT [DF__TM_Tkt__UpdateDa__6A30C649] DEFAULT (getdate()),
...
GO
ALTER TABLE [ods].[TM_Tkt] ADD CONSTRAINT [PK__TM_Tkt__3213E83E3342C6C1] PRIMARY KEY NONCLUSTERED  ([id])
GO
CREATE NONCLUSTERED INDEX [IDX_PlanKey] ON [ods].[TM_Tkt] ([acct_id], [plan_event_id], [ticket_status], [section_id], [row_id], [seat_num]) INCLUDE ([num_seats], [ssbPriceCode])
GO
CREATE NONCLUSTERED INDEX [IDX_ticket_status] ON [ods].[TM_Tkt] ([ticket_status])
GO
CREATE NONCLUSTERED INDEX [<Name of Missing Index, sysname,>] ON [ods].[TM_Tkt] ([ticket_status], [add_datetime]) INCLUDE ([block_purchase_price], [event_id], [event_name], [num_seats], [plan_event_id], [price_code], [Price_code_desc], [row_id], [seat_num], [section_id])
GO
CREATE NONCLUSTERED INDEX [IX_TMTKT_SalesByDateRPT] ON [ods].[TM_Tkt] ([ticket_status], [event_id], [seat_num], [section_id], [row_id]) INCLUDE ([id], [num_seats])
GO
CREATE CLUSTERED INDEX [Idx_TM_Tkt_upd_datetime_event_id] ON [ods].[TM_Tkt] ([upd_datetime], [acct_id], [event_id])
GO
CREATE NONCLUSTERED INDEX [IDX_UpdateDate] ON [ods].[TM_Tkt] ([UpdateDate])
GO
